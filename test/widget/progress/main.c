...
int main() {
    ...
    vnw_second.pos_x = vnw_first.pos_x + vnw_first.width + 4;
    vnw_second.pos_y = vnw_first.pos_y;

    VN_UI_WIDGET vnw_third;
    vnw_third.width  = 10;
    vnw_third.height = 5;
    vnw_third.pos_x = vnw_second.pos_x + vnw_second.width + 4 ;
    vnw_third.pos_y = vnw_second.pos_y;
    /* --------------- */

    /* TESTING CODES */
    vn_code("screen:clear");
    vn_progress(20, '|', '#', '.', vnt_red, vnw_first);
    vn_progress(10, 'I', 'X', ',', vnt_green, vnw_second);
    vn_progress(2, '1', 'O', 'o', vnt_blue, vnw_third);
    /* ------------- */
