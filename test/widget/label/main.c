...
int main() {
    ...
    vnw_second.pos_x = vnw_first.pos_x + vnw_first.width + 1;
    vnw_second.pos_y = vnw_first.pos_y;

    VN_UI_WIDGET vnw_third;
    vnw_third.width  = 10;
    vnw_third.height = 5;
    vnw_third.pos_x = vnw_second.pos_x + vnw_second.width + 1 ;
    vnw_third.pos_y = vnw_second.pos_y;
    /* --------------- */

    /* TESTING CODES */
    vn_code("screen:clear");
    vn_label("Train yourself to let go of everything you fear to lose.", vnt_red, vnw_first);
    vn_label("Try not. Do or do not. There is no try.", vnt_green, vnw_second);
    vn_label("In a dark place we find ourselves and a little more knowledge lights our way.", vnt_blue, vnw_third);
    /* ------------- */
